...
public class RuneEntityLoader {
	 ...
	 */
	public RuneEntity load(String entityName) {
		...
		try {
			...
			if(properties.containsKey(RuneEntity.PROP_BASE)) {
				Logger.getLogger(RuneEngine.LOGGER_SUBSYSTEM).finer("loading base entity = "+properties.get(RuneEntity.PROP_BASE));
			...
			Logger.getLogger(RuneEngine.LOGGER_SUBSYSTEM).severe("InvalidEntity: invalid file format (cause: "+ex.getMessage()+")");
			throw new InvalidEntityException("invalid file format", ex);
		} catch (YamlException ex) {
			Logger.getLogger(RuneEngine.LOGGER_SUBSYSTEM).severe("InvalidEntity: error in entity description (cause: "+ex.getMessage()+")");
